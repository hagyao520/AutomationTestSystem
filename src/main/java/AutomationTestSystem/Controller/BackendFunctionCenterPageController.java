...
//            DatabaseUtil.Connect_MySql(djdbcDriver, dhostAddress, dport, ddataBase, duserName, dpassWord);
            ...
            if (!moreRecords) {
                ...
                for (int i = 1; i <= rsmd.getColumnCount(); ++i) {
                    // columnHeads.addElement(rsmd.getColumnName(i)); // 获得原始列名(将列名存放至向量columnHeads)
    ...
    public static void TableView(JTable tabDemo) {
        ...
        // // 设置表头的字体
        tabDemo.getTableHeader().setFont(Constants.BASIC_FONT3);
        // // 设置表头文字的颜色
        tabDemo.getTableHeader().setForeground(Color.white);
        // // 设置表单的表头高度
        // tabDemo.getTableHeader().setSize(30, 150);
        // // 设置表头的背景色
